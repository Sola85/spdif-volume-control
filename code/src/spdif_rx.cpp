...
extern SerialUSB _serialUSB;
//PIO_DREQ_NUM(pio, sm, is_tx) ((sm) + (((is_tx) ? 0 : NUM_PIO_STATE_MACHINES) + PIO_NUM(pio) * (DREQ_PIO1_TX0 - DREQ_PIO0_TX0)))

#define SYSTEM_CLK_FREQUENCY (192000000)
...
static uint trans_count = SPDIF_BLOCK_SIZE_;
static uint8_t c_bits[SPDIF_BLOCK_SIZE_ / 16];
...
__attribute__((weak))
void spdif_rx_callback_func(uint32_t* buff, uint32_t sub_frame_count, uint8_t c_bits[SPDIF_BLOCK_SIZE_ / 16], bool parity_err)
{
    return;
}

static int _check_block(uint32_t buff[SPDIF_BLOCK_SIZE_])
{
    uint pos_syncB = 0;
    uint32_t block_parity_err_count = 0;
    uint8_t c_bits_raw[SPDIF_BLOCK_SIZE_ / 16];
    ...
    if (block_aligned) {
        ...
        if (pos_syncB != 0 && block_align_count == 0) {
            // dispose pos_syncB to align because fifo_buff[SPDIF_BLOCK_SIZE_-1] was (SPDIF_BLOCK_SIZE_ - pos_syncB)'th sub frame
            // it takes 3 blocks to align because coming 2 transfers already issued
            trans_count = (pos_syncB > SPDIF_BLOCK_SIZE_ / 2) ? pos_syncB : SPDIF_BLOCK_SIZE_ / 2; // workaround: small number of trans_count can make DMA overun
            block_align_count = 3;
        } else {
            trans_count = SPDIF_BLOCK_SIZE_;
...
static uint32_t _dma_done_and_restart(uint8_t dma_channel, dma_channel_config* dma_config)
{
    ...
    if (spdif_rx_get_state() == SPDIF_RX_STATE_STABLE) {
        if (spdif_rx_get_fifo_count() + SPDIF_BLOCK_SIZE_ > SPDIF_RX_FIFO_SIZE) {
            //printf("spdif_rx fifo overflow\n");
            buff_rd_ptr = _ptr_inc(buff_rd_ptr, SPDIF_BLOCK_SIZE_); // dispose overflow data
        }
        buff_wr_done_ptr = _ptr_inc(done_ptr, SPDIF_BLOCK_SIZE_);
    ...
    buff_wr_pre_ptr = _ptr_inc(buff_wr_pre_ptr, SPDIF_BLOCK_SIZE_);
    return done_ptr;
}
...
static void _spdif_rx_capture_start()
{
    ...
    channel_config_set_dreq(&dma_config0, pio_get_dreq(spdif_rx_pio, gcfg.pio_sm, false));
...
static void _spdif_rx_decode_start(spdif_rx_samp_freq_t samp_freq, bool inverted)
{
    ...
    channel_config_set_read_increment(&dma_config0, false);
    channel_config_set_write_increment(&dma_config0, true);
    channel_config_set_dreq(&dma_config0, pio_get_dreq(spdif_rx_pio, gcfg.pio_sm, false)); //PIO_DREQ_NUM(pio, sm, is_tx) ((sm) + (((is_tx) ? 0 : NUM_PIO_STATE_MACHINES) + PIO_NUM(pio) * (DREQ_PIO1_TX0 - DREQ_PIO0_TX0))) //pio_get_dreq(_pio, _sm, false)//DREQ_PIOx_RX0 + gcfg.pio_sm
    ...
    channel_config_set_dreq(&dma_config1, pio_get_dreq(spdif_rx_pio, gcfg.pio_sm, false));
...
void __isr __time_critical_func(spdif_rx_dma_irq_handler)()
{
    ...
    { // Calculate samp_freq and check if it's stable
        ...
        float bitrate_16b = (float) SPDIF_BLOCK_SIZE_ * 2 * 8 * 1e6 / ave_block_interval;
...
void spdif_rx_get_c_bits(void* ptr, size_t size, uint32_t offset)
{
    ...
    for (int i = 0; i < size; i++) {
        if (offset + i >= SPDIF_BLOCK_SIZE_ / 16) {
            break;
        }
